<script lang="ts">
	...
	const navItems = [
		...
		{ icon: Flame, label: 'Kebiasaan', path: '/ibadah' },
...
<nav class="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-base-content/5 bg-base-100/90 px-2 py-2 pb-safe backdrop-blur-xl md:hidden {className} shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-[0_-4px_20px_rgba(0,0,0,0.3)]">

	{#each navItems.slice(0, 2) as item}
		<a href="{item.path}" class="group flex flex-1 flex-col items-center justify-center gap-1 p-1 transition-colors {isActive(item.path) ? 'text-primary' : 'text-base-content/40'}">
			<item.icon size={22} />
