<script lang="ts">
	...
	import { Flame, Clock, Calendar, Check, BookOpen, Activity, ArrowRight, Sparkles, Heart, RefreshCw } from 'lucide-svelte';
	...
	<div class="max-w-4xl mx-auto space-y-8">
		...
		<div class="grid md:grid-cols-3 gap-6">
			<div class="md:col-span-2 card bg-linear-to-br from-primary to-primary-focus text-primary-content shadow-xl overflow-hidden relative" in:scale={{ duration: 600, start: 0.95, delay: 100 }}>
				<div class="absolute top-0 right-0 -mt-10 -mr-10 size-40 rounded-full bg-white/50 blur-3xl"></div>
				
				{#if timer.nextPrayer}
					<div class="card-body p-8 text-center relative z-10">
						<Badge class="bg-white/20 text-white/80 px-3 py-1 rounded-full flex items-center gap-2 mx-auto mb-4">
							<Clock class="size-4" />
						...
						<div class="flex justify-between items-center">
							<Badge class="bg-white/20 text-white/80 px-3 py-1 rounded-full flex items-center gap-2">
								<Clock class="size-4" />
								<span>{timer.nextPrayer.time}</span>
							</Badge>
							<Button onclick={() => goto('/jadwal-solat')} class='rounded-xl' size='sm' variant='primary'>Jadwal Sholat</Button>
						</div>
					</div>
				{:else}
					<div class="p-8 text-center relative z-10 flex flex-col items-center h-full justify-center gap-4">
				...
				<div class="card-body p-6">
					...
					<div class="space-y-4">
						<div>
							...
							<progress class="progress progress-primary w-full" value={profile?.progress?.ibadah ?? 0} max="100"></progress>
						</div>
						<div>
							<div class="flex justify-between text-sm mb-1">
								<span>Kebiasaan</span>
							...
							<progress class="progress progress-secondary w-full" value={profile?.progress?.habits ?? 0} max="100"></progress>
						</div>
						<div>
							<div class="flex justify-between text-sm mb-1">
								<span>Quran</span>
							...
							<progress class="progress progress-accent w-full" value={profile?.progress?.quran ?? 0} max="100"></progress>
						</div>
					</div>
				</div>
			</div>
