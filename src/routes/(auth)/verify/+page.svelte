<script lang="ts">
    ...
    const features = $derived([
        m.otpFeatureEncryption(),
        m.otpFeatureDataProtection(),
        m.otpFeatureSecureAccess(),
        m.otpFeatureFastRecovery()
    ]);
...
<div class="min-h-screen flex items-center justify-center p-4 bg-base-200">
    ...
    >
            ...
            <div class="card-body gap-6">
                ...
                <div class="text-center space-y-2" in:fly={{ y: 15, duration: 400, delay: 150 }}>
                    <h1 class="text-2xl font-bold">
                        {isSuccess ? m.otpTitleSuccess() : m.otpTitle()}
                    </h1>
                    <p class="text-base-content/60 text-sm">
                        {#if isSuccess}
                            {m.otpSubtitleSuccess()}
                        {:else}
                            <span class="inline-flex items-center gap-2">
                                <Mail class="w-4 h-4" />
                                {m.otpSubtitlePending({ email: data.email })}
                            </span>
                        {/if}
                    </p>
                </div>
                        ...
                        use:enhance={() => {
                            ...
                            return async ({ result }) => {
                                ...
                                if (result.type === 'success') {
                                    isSuccess = true;
                                    toast.add(m.otpSuccessToast(), 'success', 3000);
                                    fireConfetti();
                                    setTimeout(() => {
                                        goto('/change-password');
                                    }, 2500);
                                } else if (result.type === 'failure') {
                                    toast.add(m.otpInvalidToast(), 'error', 3000);
                                    triggerShake();
                                    otp = ['', '', '', '', '', ''];
                                    otpInputs[0]?.focus();
                                }
                    ...
                    <form 
                        ...
                        use:enhance={() => {
                            ...
                            return async ({ result }) => {
                                isResending = false;
                                if (result.type === 'success') {
                                    toast.add(m.otpResendSuccessToast(), 'success', 3000);
                                    startResendCountdown();
                                } else if (result.type === 'failure') {
                                    toast.add(m.otpResendFailedToast(), 'error', 3000);
                                }
                            };
                        }}
                        class="text-center"
                    >
                        ...
                        >
                            ...
                            {:else if canResend}
                                {m.otpResend()}
                            {:else}
                                {m.otpResendWaiting({ seconds: countdown.toString() })}
                            {/if}
                        </button>
                    </form>
                {:else}
                    ...
                    >
                        ...
                        <p class="text-base-content/60 mt-2">{m.otpSuccessRedirect()}</p>
                    </div>
                {/if}
            </div>
        </div>
        ...
        <p class="text-center text-base-content/50 text-xs mt-6" in:fly={{ y: 15, duration: 400, delay: 300 }}>
            &copy; {new Date().getFullYear()} {m.otpFooter()}
        </p>
    </div>
</div>
